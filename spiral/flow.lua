...
function FlowMap:spiral()
    ...
    for y = 1, self.n do
        ...
        for x = 1, self.n do
            ...
            local vector = Vector:create(xc - xOffset, yc - yOffset)
            vector:norm()
            local noiseValue = love.math.noise(xOffset, yOffset)
            local t = math.map(noiseValue, 10, 1, 0, math.pi * 2)


            self.field[y][x] = Vector:create(
                vector.x * math.cos(rotationAngle + t) - vector.y * math.sin(rotationAngle + t),
                vector.y * math.cos(rotationAngle + t) + vector.x * math.sin(rotationAngle + t)
            )
